...
import { Activity, Bell, Check, ChevronDown, Command, Database, Gauge, Grid3X3, Layers3, Search, Settings, SlidersHorizontal, Terminal } from 'lucide-react';
...
const Button = React.forwardRef(function Button({ children, variant='primary', loading, disabled, ...rest }, ref) {
  return <button ref={ref} disabled={disabled || loading} className={cx('u-button', `u-button--${variant}`, loading && 'is-loading')} {...rest}>{loading ? 'Cargando' : children}</button>;
});
...
function SegmentedControl() {
  ...
  const opts = ['Compact', 'Comfortable'];
  return (
    <div className="u-segmented" role="group" aria-label="Densidad">
      {opts.map(o => (
        <button
          key={o}
          type="button"
...
function Tabs() {
  ...
  const opts = ['Overview', 'Signals', 'Logs'];
  return (
    <div className="u-tabs" role="tablist" aria-label="Vistas">
      {opts.map(o => (
        <button
          key={o}
          type="button"
...
function Checkbox({ defaultChecked = false }) {
  ...
  return (
    <label className="u-check">
      <input type="checkbox" checked={checked} onChange={(e) => setChecked(e.target.checked)} className="u-check__input"/>
      <span aria-hidden="true" className={checked ? 'is-checked' : ''}>{checked && <Check size={12}/>}</span>
      Sincronizar estado
    </label>
  );
}
function Toggle({ defaultOn = false }) {
  ...
  return (
    <button
      ...
      className={cx('u-toggle', on && 'is-on')}
    ><i/></button>
  );
}
function Select() {
  return (
    <label className="u-field">
      ...
      <div className="u-select">
        <select className="u-select__native" defaultValue="lab" aria-label="Modo">
...
function Tooltip() {
  const id = useId();
  return (
    <span className="u-tooltip" tabIndex={0} aria-describedby={id}>
      hover
      <span id={id} role="tooltip">Señal contextual, no decoración.</span>
    </span>
  );
}
function Modal() {
  ...
  return (
    <>
      <Button ref={triggerRef} onClick={() => setOpen(true)}>Abrir modal</Button>
      {open && (
        <div className="u-modal" role="dialog" aria-modal="true" aria-labelledby="umbral-modal-title">
          <div ref={dialogRef}>
            <h3 id="umbral-modal-title">Confirmar acción</h3>
            <p>Los modales son contenidos, sobrios y escasos.</p>
...
function Toast() { return <div className="u-toast" role="status" aria-live="polite"><Status status="OK"/>Snapshot guardado</div>; }
...
function ComponentGrid() { return <div className="component-grid"><Panel title="Buttons"><div className="stack"><Button>Primary</Button><Button variant="secondary">Secondary</Button><Button variant="ghost">Ghost</Button><Button disabled>Disabled</Button><Button loading>Loading</Button></div></Panel><Panel title="Inputs"><Input/><Input error/><Select/><SegmentedControl/></Panel><Panel title="Controls"><Tabs/><Checkbox defaultChecked/><Checkbox/><div className="inline"><Toggle defaultOn/><Toggle/></div><Slider/><ParamControl/></Panel><Panel title="Signals"><div className="inline wrap"><Badge>ACTIVE</Badge><Badge tone="success">OK</Badge><Badge tone="warning">WARN</Badge><Badge tone="error">ERROR</Badge><Tag>parametric</Tag><Tooltip/></div><Divider/><Progress/><Toast/></Panel><div style={{marginTop: 'var(--space-6)'}}><Modal/></div></div>; }
